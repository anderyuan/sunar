...
namespace Esp.Auth.Tests
{
    public abstract class AuthTestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<AuthDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<AuthDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AuthDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AuthDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AuthDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AuthDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<AuthDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AuthDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AuthDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AuthDbContext, Task<T>> func)
        {
            T result;
